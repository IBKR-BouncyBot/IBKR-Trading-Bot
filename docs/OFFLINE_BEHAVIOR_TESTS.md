...
This document describes the current non-GUI, non-Windows, non-network test layer in v3.0.17. It covers strategy behavior, controller state transitions, broker-event handling, persistence and recovery, shutdown checkpoints, GUI contracts, and bounded performance behavior.
...
The current v3.0.17 validation inventory is:

- 806 non-soak tests: 805 expected passes and 1 strict expected failure documenting the Master Client/shared-prefix limitation;
- 5 bounded soak tests;
- 811/811 effective executable application callables entered;
- 76.4% measured combined statement/branch coverage against the 75% minimum;
